...
const ActivityLog = ({ cardId = null }) => {
  ...
  return (
    <div className='activity-log'>
      {cardId && (
        <button className='modal-btn' onClick={toggleContent}>
        ...
        list.map((activity) => (
          <div className='activity-content' key={activity.id}>
            {cardId ? (
              ...
              <Markdown className='activity-desc' id='boardActivity' options={{ forceBlock: true }}>
                {activity.boardTxt}
              </Markdown>
            )}
      ...
      {showContent && showMore && activities.length > LIMIT && (
        <button className='clear-btn show-more-btn' onClick={loadMore}>
          {" "}
          Load More{" "}
        </button>
      )}
